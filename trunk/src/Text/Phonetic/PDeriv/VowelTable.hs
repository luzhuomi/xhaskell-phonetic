-- The Vowel table records the similarity between the consonants (found in english), which can be used to match against mispelled words or singlish
-- The mkVowelPattern function construct a pattern which accept consonants
module Text.Phonetic.PDeriv.VowelTable where
...
type VowelTable = D.Dictionary [(S.ByteString, Float)]
...
parseVowelTable :: FilePath -> IO VowelTable 
parseVowelTable fp = do 
  { c <- S.readFile fp
  ; let lns = S.lines c
  ; case lns of 
  ...
  where forEachRow :: [S.ByteString] -> VowelTable -> S.ByteString -> VowelTable
        forEachRow initials tab ln = 
          let (vowel:scores) = bs_splitBy' isTab ln
              initials_scores = map (\(x, Just y) -> (x,y)) (filter (\p -> isJust (snd p)) (zip initials (map parseScore scores)))
          in D.insert vowel initials_scores tab
